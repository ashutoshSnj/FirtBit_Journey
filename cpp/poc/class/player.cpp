...
class Player{
	...
	virtual void play() {
    cout<<this->name<<" is a player and is preparing to play."<<endl;
}
...
class Cricketplayer:public Player{
   ...
   public:
	 ...
	 Cricketplayer():Player(){
	  ...
	  void play(){
    cout<<this->getname()<<" is playing cricket,scoring runs and taking wickets."<<endl;
}
...
class Football:public Player{
	...
	void play(){
    cout<<this->getname()<<" is playing football, aiming to score goals from the "<<this->position<<"position."<<endl;
} 
};
int main(){
Player *ptr;
