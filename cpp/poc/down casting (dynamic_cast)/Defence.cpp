...
class Defence{
    ...
    virtual void attack() { 
        cout<<"General defence attack strategy."<<endl;
    }
};//defence end here
...
void force_details(Defence*ptr){
  Airforce *str=dynamic_cast<Airforce*>(ptr);
   if(str!=NULL){
   	cout<<"no of aorcraft "<<str->getaircrafts()<<endl;
   }
   else{
   	cout<<"Airforce not present "<<endl;
   }
   Army *arm=dynamic_cast<Army*>(ptr);
   if(arm!=NULL){
   	cout<<"no of slodires "<<arm->getsoldiers()<<endl;
   }
   else{
   	cout<<"army not present"<<endl;
   }
   // static cast or blind cast
  Navy*nrt=static_cast<Navy*>(ptr);
  	cout<<nrt->getships();
}
int main(){
	Defence*ptr;
